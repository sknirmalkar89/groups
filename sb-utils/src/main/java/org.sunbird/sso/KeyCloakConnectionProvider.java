...
public class KeyCloakConnectionProvider {
  ...
  public static Keycloak initialiseConnection() throws Exception {
    logger.debug("key cloak instance is creation started.");
    ...
    logger.debug("key cloak instance is created successfully.");
    return keycloak;
  }
  ...
  static class ResourceCleanUp extends Thread {
    public void run() {
      logger.debug("started resource cleanup.");
      if (null != keycloak) {
        keycloak.close();
      }
  ...
  public static void registerShutDownHook() {
    ...
    logger.debug("ShutDownHook registered.");
  }
}
