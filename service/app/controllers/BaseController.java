...
public class BaseController extends Controller {
  ...
  public CompletionStage<Result> handleRequest(
      ...
      Request request,
      RequestValidatorFunction validatorFunction,
      String operation) {
    try {
      ...
      return CompletableFuture.supplyAsync(() -> StringUtils.EMPTY)
          .thenApply(
              result -> badRequest(Json.toJson(RequestHandler.prepareFailureMessage(ex, req))));
    } catch (Exception ex) {
      return CompletableFuture.supplyAsync(() -> StringUtils.EMPTY)
          .thenApply(
              result ->
                  internalServerError(Json.toJson(RequestHandler.prepareFailureMessage(ex, req))));
    }
  }
  ...
  public CompletionStage<Result> handleLogRequest() {
    ...
    //        } catch (Exception ex) {
    //            ProjectLogger.log(String.format("%s:%s:exception occurred in mapping request",
    // this.getClass().getSimpleName(), "handleLogRequest"), LoggerEnum.ERROR.name());
    ...
    //            } else {
    //                List<Enum> supportedLogLevelsValues = new
    // ArrayList<>(EnumSet.allOf(LoggerEnum.class));
    //                response.put(JsonKey.ERROR, true);
    //                response.put(
    //                        JsonKey.MESSAGE,
    //                        "Valid Log Levels are " +
    // Arrays.asList(supportedLogLevelsValues.toArray()));
    //            }
    //        } else {
    //            response.put(JsonKey.ERROR, true);
    //            response.put(
